...
export default function Page() {
  ...
  function handleAnalyze() {
    setIsLoading(true)
    setHasResults(false)
    // Simulated analysis — no backend logic per requirements.
    window.setTimeout(() => {
      setIsLoading(false)
      setHasResults(true)
      window.setTimeout(() => {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section id="analyze" ref={formRef} className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        <AnalysisForm
      ...
      {isLoading ? (
        <div className="mx-auto flex max-w-3xl flex-col items-center gap-4 px-4 pb-24 text-center sm:px-6">
          ...
          <div>
            <p className="text-lg font-semibold text-foreground">Reviewing your architecture</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Amazon Bedrock is evaluating security, scalability, reliability, cost, and readiness…
            </p>
          </div>
        </div>
      ) : null}
      ...
      {hasResults ? (
        <div ref={resultsRef} className="scroll-mt-16">
          <ResultsSection projectName={projectName} />
        </div>
      ) : null}
      ...
      <footer className="border-t border-border bg-secondary/40">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-2 px-4 py-8 text-sm text-muted-foreground sm:flex-row sm:px-6">
          <p>
            <span className="font-semibold text-foreground">ArchLens AI</span> — AWS Architecture
            Review Assistant
          </p>
